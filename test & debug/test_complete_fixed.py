#!/usr/bin/env python3
"""
Fixed test script to complete inventory session 28 and verify it appears in reports
"""
...
def test_complete_inventory():
    ...
    csrf_patterns = [
        r'id="csrf-token" value="([^"]+)"',  # New format
        r'name="csrf_token" value="([^"]+)"',  # Old format
        r'csrf_token.*?value="([^"]+)"'  # Generic pattern
    ]
    
    csrf_token = None
    for pattern in csrf_patterns:
        csrf_match = re.search(pattern, execution_response.text)
        if csrf_match:
            csrf_token = csrf_match.group(1)
            print(f'CSRF token found with pattern: {pattern}')
            break
    ...
    if not csrf_token:
        ...
        # Let's try to see what's in the page
        print('Page content sample:')
        print(execution_response.text[:1000])
        return False

    # Complete the inventory session using the correct route format
    complete_data = {'csrf_token': csrf_token}
    complete_response = session.post('http://127.0.0.1:5000/md3/inventory/sessions/28/complete', 
                                   data=complete_data,
...
if __name__ == '__main__':
    print('=== Testing Inventory Completion Workflow (Fixed) ===')
